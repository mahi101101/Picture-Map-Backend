...
const captureMap = require("../util/location-capture");
...
const getMapImg = async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new HttpError("Failed to fetch map for the provided address.", 500)
    );
  }
  res.json({
    img: img,
...
const updatePlace = async (req, res, next) => {
  ...
  if (place.creator.toString() !== req.userData.userId) {
    const error = new HttpError(
      "Something went wrong, not allowed to edit this place",
      401
    );
    return next(error);
  }
...
const deletePlace = async (req, res, next) => {
  ...
  if (place.creator.id.toString() !== req.userData.userId) {
    const error = new HttpError(
      "Something went wrong, not allowed to delete this place",
      401
    );
    return next(error);
  }
